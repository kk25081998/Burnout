...
from app.personalization import (
    save_picture,
    send_contact_email,
    user_took_test_this_month,
    process_test_results,
    get_burnout_trends,
    get_department_burnout_data,
    get_team_burnout_data,
    save_company_logo,
    create_departments,
...
def register_individual():
    ...
    return render_template(
        "register_individual.html", title="Register as Individual", form=form
    )
...
def register_company():
    ...
    return render_template(
        "register_company.html", title="Register Your Company", form=form
    )
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user.first_login:
            ...
            )  # Redirect to password change form on first login
...
def dashboard():
    ...
    last_score = (
        Results.query.filter_by(user_id=current_user.id)
        .filter(Results.testDate <= today_date)
        .order_by(Results.testDate.desc())
        .first()
    )
    last_three_scores = (
        Results.query.filter(Results.user_id == current_user.id)
        .filter(
            Results.testDate <= last_day_of_last_month
        )  # Exclude scores with future dates.
        .order_by(Results.testDate.desc())
        .limit(3)
        .all()
    )
...
def view_profile():
    ...
    image = (
        ...
        else url_for("static", filename="images/profile.jpg")
    )
        ...
        try:
            ...
            if "image" in request.files and request.files["image"].filename != "":
                picture_file = save_picture(
                    form.image.data, current_user.email, current_user.id
                )
...
def test():
    ...
    if user_took_test_this_month(current_user.id):
        next_month = datetime(
            datetime.now().year, datetime.now().month % 12 + 1, 1
        )  # First day of next month
        flash(
            f'You can only take the burnout test once per calendar month. Please come back on {next_month.strftime("%B %d, %Y")} to take the test again.',
            "warning",
        )
        return render_template(
            "test_not_eligible.html",
            title="test",
            form=form,
            next_eligible_date=next_month,
        )
...
def feedback():
    ...
    if form.validate_on_submit():
        new_feedback = Feedback(
            company_id=current_user.companyId, description=form.feedback.data
        )
        db.session.add(new_feedback)
        db.session.commit()
        return redirect(
            url_for("main.feedback", success=True)
    ...
    return render_template(
        "feedback.html", form=form, message=message, message_type=message_type
    )
...
def resources():
    ...
    evaluate_resources = Resources.query.filter_by(
        category="Evaluate Your Options"
    ).all()
    support_resources = Resources.query.filter_by(category="Seek Support").all()
    activity_resources = Resources.query.filter_by(
        category="Try a Relaxing Activity"
    ).all()
...
def contact_us():
    ...
    if form.validate_on_submit():
        ...
        response_code = send_contact_email(
            name, email, telephone, reason, formatted_message
        )
        ...
        if response_code == 202:
            ...
            form.telephone.data = ""
            form.reason.data = (
                "Select Reason"  # Reset the dropdown to the default option
            )
            form.message.data = ""
...
def company_dashboard():
    ...
    adminCount = User.query.filter_by(
        companyId=current_user.companyId, role_id=2
    ).count()
    hrCount = User.query.filter_by(companyId=current_user.companyId, role_id=3).count()
    managerCount = User.query.filter_by(
        companyId=current_user.companyId, role_id=4
    ).count()
    userCount = User.query.filter_by(
        companyId=current_user.companyId, role_id=5
    ).count()
        ...
        for role, user_ids in role_ids_mapping.items():
            results = (
                db.session.query(
                    ...
                    func.round(
                        100
                        * func.avg(Results.scoreA + Results.scoreB + Results.scoreC)
                        / 132
                    ).label("avg_value"),
                )
...
def company_users():
    ...
    if current_user.role_id == 3:
        company_users = User.query.filter_by(companyId=current_user.companyId).filter(
            User.role_id.in_([4, 5])
        )
        manager_id_to_email = {user.id: user.email for user in User.query.all()}
        return render_template(
            "hrusermanagement.html",
            users=company_users,
            manager_emails=manager_id_to_email,
        )
...
def edit_user(user_id):
    ...
    if request.method == "POST":
        ...
        if not form.validate():
            # print(form.errors)
            return render_template(
                "edituser.html", title="Edit User", form=form, user=user
            )
...
def delete_user(user_id):
    ...
    return redirect(
        url_for("main.company_users")
    )  # Or whichever route you want to redirect to after deletion
            ...
            results = (
                db.session.query(
                    ...
                    func.round(
                        100
                        * func.avg(
                            Results.scoreA + Results.scoreB + (48 - Results.scoreC)
                        )
                        / 132
                    ).label("avg_value"),
                )
...
def hr_burnout_report():
    ...
    def get_all_departments(company_id):
        ...
        distinct_departments = (
            db.session.query(distinct(User.department))
        ...
        departments = [
            department[0] for department in distinct_departments if department[0]
        ]  # the if condition ensures we don't include None values
...
def hr_feedback():
    ...
    feedbacks = (
        Feedback.query.filter_by(company_id=current_user.companyId)
...
def createuser():
    ...
    if form.validate_on_submit():
        ...
        if current_user.role_id == 3:
            company_users = (
                User.query.filter_by(companyId=current_user.companyId)
