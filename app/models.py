...
class User(UserMixin, db.Model):
    ...
    )  # changed from db.Date to db.String
    ...
    subordinates = db.relationship(
        "User", backref=db.backref("manager", remote_side=[id]), lazy="dynamic"
    )
...
class Feedback(db.Model):
    ...
    )  # Text field to allow longer feedback
    status = db.Column(
        db.String(64), default="Pending"
    )  # Useful to track feedback status
...
class PaymentHistory(db.Model):
    ...
    )  # The ID from the payment gateway, useful for potential refunds or tracking
